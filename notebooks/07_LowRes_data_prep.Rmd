...
```{r}
pig_d <- read_excel("~/SourceData/Pig.xlsx", sheet = "pig_area")
prod <- read_excel("~/SourceData/Pig.xlsx", sheet = "pig_prod")
...
#low resolution
write.xlsx(df_clustered, "~/Pig_clustered.xlsx", rowNames = FALSE, col_type ="numeric")
```
...
```{r}
PCA_pig <- read_excel("~/Pig_clustered.xlsx")
...
alr_safe<- bind_cols("depth" = PCA_pig$`Composite depth`, t(alr))

write.xlsx(alr_safe, "~/pig_alr.xlsx", rowNames = FALSE, col_type ="numeric")
```
...
write.xlsx(df_PC, "~/Pig_clustered_PC.xlsx", rowNames = FALSE, col_type ="numeric")
```
...
```{r}
pig_prod <- read_excel("~/SourceData/Pig.xlsx", sheet = "pig_prod")
...
```{r}
alr_safe <- read.xlsx("~/pig_alr.xlsx")
```
...
```{r}
ann_colors = list(
   producer = c(Cryptophytes = "darkorange3", Cyanobacteria ="royalblue2", `Green algae` = "green4", # `Green sulphur bacteria` = "greenyellow", `Purple non-sulphur bacteria`= "orchid2", `Purple sulphur bacteria` = "purple2", `Red blooming algae` = "red4", 
                Silicifiers= "lightskyblue"))#,
...
png(filename = "~/figs/heatmap_corr_clustered.png", width = 1200, height = 800, units = "px", pointsize = 18)
...
png(filename = "~/figs/Heatmap_corr.png", width = 1200, height = 800, units = "px", pointsize = 18)
obj <- pheatmap(alr_scaled, show_rownames=T,show_colnames=T,
         annotation_col=annotation_col, #nnotation_row = annotation_row,
         annotation_colors = ann_colors,
...
write.xlsx(alr_scaled_clust, "~/pig_alr_clust.xlsx", rowNames = FALSE, col_type ="numeric")
```
